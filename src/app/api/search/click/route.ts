...
async function getScopedSupabaseWithUser() {
...
export const POST = withSentryRoute(async (request: NextRequest) => {
  ...
  if (clientIdentifier !== 'unknown') {
    const ipRate = await checkRateLimit(`search-click:ip:${clientIdentifier}`, CLICK_RATE_LIMIT_PER_IP);
    if (!ipRate.success) {
  ...
  const { supabase, userId } = await getScopedSupabaseWithUser();
