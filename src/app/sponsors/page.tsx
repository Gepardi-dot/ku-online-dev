...
export default async function SponsorsPage({ searchParams }: { searchParams?: Promise<SponsorsPageSearchParams> }) {
  ...
  const canManageStores = isAdmin(user);
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-black/5 pt-4">
                    {canReviewApplications ? (
                      ...
                      >
                        ...
                        {sellerApplicationsCount !== null ? ` (${sellerApplicationsCount})` : ''}
                      </Link>
                    ) : null}
                    {canManageStores ? (
                      <Link
                        ...
                        dir="auto"
                      >
                        {serverTranslate(locale, 'sponsorsHub.manageStoresButton')}
                      </Link>
                    ) : null}
                  </div>
                ) : null}
            ...
            {spotlightStores.length ? (
              <div>
                <div className="mb-3 flex flex-wrap items-center gap-2">
                  <h2 className="text-lg font-extrabold text-[#2D2D2D] md:text-xl" dir="auto">
