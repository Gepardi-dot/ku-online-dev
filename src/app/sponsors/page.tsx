...
import { listSponsorOfferPreviewsByStoreIds, listSpotlightSponsorStores, type SponsorOfferPreview } from '@/lib/services/sponsors';
...
export default async function SponsorsPage({ searchParams }: { searchParams?: Promise<SponsorsPageSearchParams> }) {
  ...
  // Dev-only preview fallback: if there are no sponsor stores yet, show realistic mock cards.
  if (process.env.NODE_ENV !== 'production' && spotlightStores.length < 8) {
    const { PROTOTYPE_STORES } = await import('@/lib/prototypes/sponsors');
    const active = PROTOTYPE_STORES.filter((s) => s.status === 'active');
    const filtered = cityFilter ? active.filter((s) => s.primaryCity === cityFilter) : active;
    const existing = new Set(spotlightStores.map((s) => s.slug));
    for (const s of filtered) {
      if (spotlightStores.length >= 8) break;
      if (existing.has(s.slug)) continue;
      spotlightStores.push(s as any);
      existing.add(s.slug);
    }
  }

  const offerByStoreId = await listSponsorOfferPreviewsByStoreIds(spotlightStores.map((s) => s.id));
  // Dev fallback: if we're showing prototype stores, also attach realistic prototype offers.
  if (process.env.NODE_ENV !== 'production') {
    const { PROTOTYPE_OFFERS } = await import('@/lib/prototypes/sponsors');
    const now = Date.now();
    const pickBest = (storeId: string): SponsorOfferPreview | null => {
      const eligible = PROTOTYPE_OFFERS.filter((o) => {
        if (o.storeId !== storeId) return false;
        if (o.status !== 'active') return false;
        const startOk = !o.startAt || o.startAt.getTime() <= now;
        const endOk = !o.endAt || o.endAt.getTime() > now;
        return startOk && endOk;
      });
      eligible.sort((a, b) => Number(b.isFeatured) - Number(a.isFeatured) || (a.endAt?.getTime() ?? Number.POSITIVE_INFINITY) - (b.endAt?.getTime() ?? Number.POSITIVE_INFINITY));
      ...
      return {
        ...
        storeId: top.storeId,
        title: top.title,
        discountType: top.discountType,
        discountValue: top.discountValue ?? null,
        currency: top.currency ?? null,
        endAt: top.endAt ?? null,
        originalPrice: top.originalPrice ?? null,
        dealPrice: top.dealPrice ?? null,
      };
    };

    for (const store of spotlightStores) {
      if (offerByStoreId[store.id]) continue;
      const preview = pickBest(store.id);
      if (preview) offerByStoreId[store.id] = preview;
    }
  }
