...
export type SponsorStoreSetupInfo = {
  id: string;
  name: string;
  slug: string;
...
export type SponsorStoreSetupSummary = {
  offerCount: number;
...
function ServiceForm({
  ...
  const [active, setActive] = useState(initial?.status ? isActiveStatus(initial.status) : true);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (showValue && discountValue.trim() !== '' && !Number.isFinite(valueNum as number)) {
...
function EditServiceDialog({
  ...
  const updateService = async (
    ...
    try {
      ...
      if (!res.ok || !data.ok) {
        const message = typeof (data as { error?: string }).error === 'string' ? (data as { error?: string }).error : null;
...
export function SponsorStoreSetupSections({
  store,
  initialItems,
  locale,
  sponsoredLabel,
  ...
  onStoreDeleted,
  className,
  showGlobalSaveButton = true,
}: {
  store: SponsorStoreSetupInfo;
  ...
  onSummaryChange?: (summary: SponsorStoreSetupSummary) => void;
  onStoreDeleted?: (storeId: string) => void;
  className?: string;
  showGlobalSaveButton?: boolean;
}) {
  const router = useRouter();
  const [items, setItems] = useState<SponsorServiceItem[]>(initialItems ?? []);
  const [submitting, setSubmitting] = useState(false);
  const [hydrating, setHydrating] = useState(Boolean(hydrateOnMount && mode === 'remote'));
  ...
  }, [initialItems, store.id]);

  useEffect(() => {
    setStoreCardUrl(store.coverUrl?.trim() || null);
    setStoreCardAspectRatio(DEFAULT_STORE_CARD_ASPECT_RATIO);
    setStorePhone(store.phone?.trim() || '');
    setStoreWhatsapp(store.whatsapp?.trim() || '');
    setStoreWebsite(store.website?.trim() || '');
    setStoreDirectionsUrl(store.directionsUrl?.trim() || '');
  }, [store.coverUrl, store.directionsUrl, store.id, store.phone, store.website, store.whatsapp]);

  useEffect(() => {
    onSummaryChange?.({
      offerCount: items.length,
      hasCardImage: Boolean(storeCardUrl),
      hasContacts: Boolean(
        normalizeNullable(storePhone) ||
          normalizeNullable(storeWhatsapp) ||
          normalizeNullable(storeWebsite) ||
          normalizeNullable(storeDirectionsUrl),
      ),
    });
  }, [items.length, onSummaryChange, storeCardUrl, storeDirectionsUrl, storePhone, storeWebsite, storeWhatsapp]);

  useEffect(() => {
    if (mode !== 'remote' || !hydrateOnMount || !store.id) {
      setHydrating(false);
      return;
    }
    ...
    const hydrate = async () => {
      try {
        const [storeRes, offersRes] = await Promise.all([
          fetch(withStoreId('/api/sponsors/store', store.id, includeStoreIdParamForStore), {
            method: 'GET',
            headers: { Accept: 'application/json' },
            cache: 'no-store',
          }),
          fetch(withStoreId('/api/sponsors/services', store.id, includeStoreIdParamForOffers), {
            method: 'GET',
            headers: { Accept: 'application/json' },
            cache: 'no-store',
          }),
        ...
        const storeData = (await storeRes.json().catch(() => ({}))) as StoreGetResponse;
        if (storeRes.ok && storeData.ok) {
          setStoreCardUrl(storeData.store.coverUrl ?? null);
          setStorePhone(storeData.store.phone ?? '');
          setStoreWhatsapp(storeData.store.whatsapp ?? '');
          setStoreWebsite(storeData.store.website ?? '');
          setStoreDirectionsUrl(storeData.store.directionsUrl ?? '');
        }

        const offersData = (await offersRes.json().catch(() => ({}))) as OffersGetResponse;
        if (offersRes.ok && offersData.ok) {
          setItems(Array.isArray(offersData.offers) ? offersData.offers : []);
        }
      } catch (error) {
        if (active) {
          console.error('Failed to hydrate sponsor store setup', error);
        }
      } finally {
        if (active) {
          setHydrating(false);
  ...
  }, [hydrateOnMount, includeStoreIdParamForOffers, includeStoreIdParamForStore, mode, store.id]);
  ...
  const deleteStore = async () => {
    ...
    try {
      ...
      onStoreDeleted?.(data.deletedStoreId);
      if (!onStoreDeleted) {
  ...
  return (
    <div className={cn('space-y-3', className)}>
      <Card className={surfaceCardClass}>
        <CardHeader className="gap-1.5 pb-2.5 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-1">
            <CardTitle className="text-base text-brand">Contacts</CardTitle>
          ...
          {hydrating ? (
            <span className="inline-flex items-center gap-1.5 rounded-full border border-black/10 bg-white/80 px-2 py-0.5 text-[10px] font-semibold text-muted-foreground">
      ...
      <Card className={surfaceCardClass}>
        <CardHeader className="gap-1.5 pb-2.5 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-1">
            <CardTitle className="text-base text-brand">Card image</CardTitle>
            <p className="max-w-2xl text-xs text-muted-foreground md:text-sm">
      ...
      <Card className={surfaceCardClass}>
        <CardHeader className="gap-1.5 pb-2.5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <CardTitle className="text-base text-brand">Offers</CardTitle>
            <span className="inline-flex rounded-full border border-black/10 bg-white/80 px-2 py-0.5 text-[10px] font-semibold text-muted-foreground">
        ...
        <CardHeader className="pb-1.5">
          <div className="flex items-end justify-between gap-3">
            <div className="min-w-0">
              <h2 className="text-sm font-extrabold text-[#111827] md:text-base">Current offers</h2>
