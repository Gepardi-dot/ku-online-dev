...
- summary: Prepared the manual provider rollout path without mutating Vercel or Algolia yet.
- details: Added `tools/scripts/algolia-search-key.mjs` to create or reuse a restricted search-only Algolia key scoped to the base product index and current replica indices. Added offline tests for scope/reuse/create behavior. Added `.github/workflows/algolia-provider-rollout.yml`, a workflow_dispatch-only job that validates existing GitHub secrets, resolves the search key without printing it, syncs all four Algolia env vars into Vercel production, optionally redeploys production, and optionally backfills the Algolia index.
- verification: `node --check tools/scripts/algolia-search-key.mjs`, `node --test tools/scripts/__tests__/algolia-search-key.test.mjs`, `npm test`, `npm run lint`, `npm run typecheck`, and `git diff --check` passed. The provider workflow has not been dispatched.
- risks: The workflow can mutate Algolia API keys, Vercel production env, Vercel production deployment, and the Algolia product index. It must not be dispatched without explicit approval of the exact `gh workflow run` command.
