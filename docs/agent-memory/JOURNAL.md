...
- verification: `node --check tools/scripts/algolia-search-key.mjs`, `node --test tools/scripts/__tests__/algolia-search-key.test.mjs`, `npm test`, `npm run lint`, `npm run typecheck`, and `git diff --check` passed before dispatch. Workflow run `28197874539` failed at search-key resolution with Algolia `403 Invalid Application-ID or API key` before Vercel env sync, deploy, or backfill.
- risks: The workflow can mutate Algolia API keys, Vercel production env, Vercel production deployment, and the Algolia product index. The current GitHub Algolia key cannot list/create API keys, so the next safe path requires adding a valid search-only key as GitHub secret `ALGOLIA_SEARCH_API_KEY` or replacing the Algolia admin/write secret with one that can manage API keys.
