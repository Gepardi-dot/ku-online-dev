...
| `vercel` CLI high advisories | Dev/deploy tooling dependency, not bundled into the user-facing app runtime. | Defer to a separate tooling slice because npm reports the fix as `vercel@54.14.2`, a major upgrade from the repo's current `^50.44.0`. |
| Remaining full-audit high advisories under Vercel CLI transitive tree | Local/deploy tooling path. | Accept temporarily until the Vercel CLI major-upgrade slice is planned, tested, and deployed through CI. |
